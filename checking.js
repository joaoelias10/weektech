 const { createClient } = supabase;
  ...
  async function loadCounters() {
    const { data } = await sb.from('participantes').select('presenca');
    if (!data) return;
    const total     = data.length;
    const presentes = data.filter(r => r.presenca === true).length;
  ...
  async function doCheckin() {
    ...
    // 3. Já fez check-in
    if (data.presenca === true) {
      setLoading(false);
      showFeedback('warning', '⚠️', 'Presença já registrada', data.nome, `RA ${ra} já realizou check-in anteriormente.`);
      document.getElementById('ra-input').value = '';
      document.getElementById('ra-input').focus();
      return;
    }

    // 4. Marca presença como true
    ...
    // 5. Sucesso!
    showFeedback('success', '✅', 'Presença confirmada!', data.nome, `Bem-vindo(a) à Tech Week 2026, ${data.nome.split(' ')[0]}!`);
