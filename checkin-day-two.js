...
async function loadCounters() {
  const { data } = await sb.from('participantes').select('presenca_dia2');
  if (!data) return;
  const total     = data.length;
  const presentes = data.filter(r => r.presenca_dia2 === true).length;
...
async function doCheckin() {
  ...
  const { data, error } = await sb
    .from('participantes')
    .select('id, nome, ra, presenca_dia2')
    .eq('ra', ra)
    .maybeSingle();
  ...
  // 3. Já fez check-in no dia 2
  if (data.presenca_dia2 === true) {
    setLoading(false);
    showFeedback('warning', '⚠️', 'Presença já registrada', data.nome, `RA ${ra} já realizou check-in no Dia 2 anteriormente.`);
    document.getElementById('ra-input').value = '';
    document.getElementById('ra-input').focus();
    return;
  }

  // 4. Marca presença_dia2 como true
  const { error: updateError } = await sb
    .from('participantes')
    .update({ presenca_dia2: true })
    .eq('id', data.id);
  ...
  // 5. Sucesso!
  showFeedback('success', '✅', 'Presença confirmada!', data.nome, `Bem-vindo(a) ao Dia 2 da Tech Week 2026, ${data.nome.split(' ')[0]}!`);
